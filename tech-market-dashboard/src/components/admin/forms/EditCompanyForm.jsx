...
const EditCompanyForm = ({ company, onSuccess, onCancel }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [logoFile, setLogoFile] = useState(null);
  const [logoPreview, setLogoPreview] = useState(company.logo ? `https://techmarket-lovat.vercel.app/${company.logo}` : null);
  const { token } = useAuth();
  ...
  useEffect(() => {
    ...
    reset({
      name: company.name,
      brief: company.brief || '',
      locationText: company.location?.text || '',
      locationLink: company.location?.link || '',
    });
  }, [company, reset]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await fetch(`https://techmarket-lovat.vercel.app/api/dealer/${company._id}`, {
        method: 'PUT',
        headers: {
          Authorization: token,
        },
