...
export default function UpdateOverEarForm({ product, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Updating over-ear headphones data...');
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update over-ear headphones: ${errorData.message || response.statusText}`);
      }
      
      const result = await response.json();
      console.log('Over-ear headphones updated successfully:', result);
      alert('Over-ear headphones updated successfully!');
  ...
  return (
    <div className="max-w-8xl mx-auto p-6 bg-white shadow-md rounded-lg space-y-6">
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">Update Over-Ear Headphones</h2>
      ...
      <div className="pt-4 flex justify-between">
        ...
        >
          {loading ? 'Updating...' : 'Update Over-Ear Headphones'}
        </button>
      </div>
    </div>
  );
