...
export default function AddOverEarForm() {
  const { register, handleSubmit, formState: { errors } } = useForm({
    defaultValues: {
      name: 'Apple AirPods Max',
      productCode: 'T2KV5CNHU4',
      referralCode: 'W4AY9RBMD6',
      brand: 'Apple',
      color: 'Purple',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      console.log('Submitting over-ear headphones data...');
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to add over-ear headphones: ${errorData.message || response.statusText}`);
      }
