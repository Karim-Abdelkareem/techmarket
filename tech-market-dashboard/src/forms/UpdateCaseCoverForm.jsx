...
export default function UpdateCaseCoverForm({ product, onClose }) {
  ...
  const onSubmit = async (data) => {
    ...
    Object.entries(finalData).forEach(([key, value]) => {
      // Handle the 'Compatible With' field with special character
      const formKey = key === 'Compatible With↵' ? 'compatibleWith' : key;
    ...
    try {
      console.log('Updating case/cover data...');
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update case/cover: ${errorData.message || response.statusText}`);
      }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-8xl mx-auto p-6 bg-white shadow-md rounded-lg space-y-6">
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">Update Case/Cover</h2>
      ...
      <div className="pt-4 flex justify-between">
        ...
        >
          {loading ? 'Updating...' : 'Update Case/Cover'}
        </button>
      </div>
    </form>
  );
