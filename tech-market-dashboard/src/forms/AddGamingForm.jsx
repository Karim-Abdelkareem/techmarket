...
const AddGamingForm = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('https://techmarket-lovat.vercel.app/api/product', {
        method: 'POST',
        headers: {
      ...
      if (onClose) onClose();
      alert('Gaming product added successfully!');
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-center">Add Gaming Product</h2>
      ...
      <button type="submit" className="btn btn-primary w-full mt-6 py-2 rounded bg-blue-600 hover:bg-blue-700 text-white font-semibold" disabled={loading}>
        {loading ? 'Adding...' : 'Add Gaming Product'}
      </button>
    </form>
  );
};

export default AddGamingForm; 