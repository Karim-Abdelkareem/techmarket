...
export default function AddWirelessForm() {
  const { register, handleSubmit, formState: { errors } } = useForm({
    defaultValues: {
      name: 'Apple AirPods Max',
      productCode: 'R9JTX2MPV7',
      referralCode: 'L5HD0ZQKJ1',
      brand: 'Apple',
      color: 'Purple',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to add wireless headphones: ${errorData.message || response.statusText}`);
      }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-8xl mx-auto p-6 bg-white shadow-md rounded-lg space-y-6">
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">Add New Wireless Headphones</h2>
