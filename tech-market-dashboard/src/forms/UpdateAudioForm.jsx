...
export default function UpdateAudioForm({ product, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Updating audio product data...');
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update audio product: ${errorData.message || response.statusText}`);
      }
      
      const result = await response.json();
      console.log('Audio product updated successfully:', result);
      alert('Audio product updated successfully!');
  ...
  return (
    <div className="max-w-8xl mx-auto p-6 bg-white shadow-md rounded-lg space-y-6">
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">Update Audio Product</h2>
      ...
      <div className="pt-4 flex justify-between">
        ...
        >
          {loading ? 'Updating...' : 'Update Audio Product'}
        </button>
      </div>
    </div>
  );
