...
export default function UpdatePowerBankForm({ product, onClose }) {
  ...
  useEffect(() => {
    if (product) {
      ...
      reset({
        ...
        color: product.color || '',
        powerBankType: product.powerBankType || '',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update power bank: ${errorData.message || response.statusText}`);
      }
      
      const result = await response.json();
      alert('Power bank updated successfully!');
      onClose();
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-8xl mx-auto p-6 bg-white shadow-md rounded-lg space-y-6">
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">Update Power Bank</h2>
      ...
      <div className="pt-4 flex justify-between">
        ...
        >
          {loading ? 'Updating...' : 'Update Power Bank'}
        </button>
      </div>
    </form>
  );
