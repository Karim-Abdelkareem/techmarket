...
export default function UpdateWirelessForm({ product, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Updating wireless headphones data...');
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update wireless headphones: ${errorData.message || response.statusText}`);
      }
      
      const result = await response.json();
      console.log('Wireless headphones updated successfully:', result);
      alert('Wireless headphones updated successfully!');
  ...
  return (
    <div className="max-w-8xl mx-auto p-6 bg-white shadow-md rounded-lg space-y-6">
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">Update Wireless Headphones</h2>
      ...
      <div className="pt-4 flex justify-between">
        ...
        >
          {loading ? 'Updating...' : 'Update Wireless Headphones'}
        </button>
      </div>
    </div>
  );
