...
export default function UpdateAccessoryForm({ product, onClose }) {
  ...
  useEffect(() => {
    if (product) {
      ...
      reset({
        ...
        'details.from': product.details?.from || '',
        'details.to': product.details?.to || '',
        'details.cableLength': product.details?.cableLength || '',
        'details.cableType': product.details?.cableType || '',
        'details.compatibleWith': product.details?.compatibleWith || '',
        'details.color': product.details?.color || '',
        'details.material': product.details?.material || ''
      });
    }
  }, [product, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update accessory: ${errorData.message || response.statusText}`);
      }
      
      const result = await response.json();
      console.log('Accessory updated successfully:', result);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-8xl mx-auto p-6 bg-white shadow-md rounded-lg space-y-6">
      <h2 className="text-2xl font-semibold text-gray-700 mb-4">Update Accessory</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          {errors.type && <span className="text-sm text-red-500 mt-1">This field is required</span>}
        </div>
        ...
        <div>
          ...
          <input 
            ...
            className="mt-1 p-2 border rounded-md w-full"
          />
          {errors.quantity && <span className="text-sm text-red-500 mt-1">This field is required</span>}
        </div>
      </div>
      ...
      {accessoryType === 'Cable' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="text-sm font-medium text-gray-600">Cable Length</label>
            <input 
              {...register('details.cableLength')} 
              className="mt-1 p-2 border rounded-md w-full"
            />
          </div>
          <div>
            <label className="text-sm font-medium text-gray-600">Cable Type</label>
            <input 
              {...register('details.cableType')} 
              className="mt-1 p-2 border rounded-md w-full"
            />
          </div>
        </div>
      ...
      {accessoryType === 'ScreenProtector' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="text-sm font-medium text-gray-600">Compatible With</label>
            <input 
              {...register('details.compatibleWith')} 
              className="mt-1 p-2 border rounded-md w-full"
            />
          </div>
          <div>
